...
<div class="container-md mx-auto sm:shadow-form bg-white p-[20px] rounded-xl mt-12">
    <form action="{{route('speakers.index')}}" method="POST" enctype="multipart/form-data">
        <p class="mb-2">Informacion Personal</p>
        @csrf
        @include('admin.ponente.formulario')

        <button href="{{route('speakers.index')}}" type="submit" class="btn-crear w-full mt-4">Registrar Ponente</button>
    </form>
</div>
@section('script')
